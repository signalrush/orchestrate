...
async def main():
    ...
    r = await auto.remind(f"The math agent computed: {a.strip()} and {b.strip()}. Summarize in one sentence.")
    ...
    assert auto._sessions["math"]["session_id"] is not None, "math session not accumulated"
    assert auto._sessions["self"]["session_id"] is not None, "self session not accumulated"
